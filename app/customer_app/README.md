...
- **Phone / OTP** — phone auth (test-number friendly).
...
## Architecture
Provider state management, a Postgres/Prisma backend accessed via REST +
Socket.IO (realtime), service layer per resource, hand-written localization
(`lib/l10n`), shared design system in `lib/utils` + `lib/widgets`.
